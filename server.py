...
def notify_payment_success(order):
    try:
        ...
        message_text = (
            f"💸 <b>Оплата заказа №{order['order_id']}</b> успешно проведена!\n\n"
            f"{client_info}\n\n"
...
def is_amount_correct(order_amount, callback_amount, payment_system):
    # Для Click и PayMe callback_amount приходит в сотых единиц, поэтому умножаем order_amount на 100 для сравнения.
